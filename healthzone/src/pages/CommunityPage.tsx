...
export function CommunityPage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/profile.php`, { credentials: 'include' })
      .then(res => res.json())
      .then(data => { if (data.status === 'success') setCurrentUser(data.user); })
      .catch(() => {});
  }, []);
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isImage && !isVideo) { alert('Only JPG, JPEG, PNG images and MP4 videos are allowed.'); return; }
    setNewMedia(file);
  ...
  const clearMedia = () => {
    setNewMedia(null); setMediaPreview(null); setMediaType(null);
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.status === 'success') {
        setShowCreateModal(false); setNewTitle(''); setNewBody(''); clearMedia();
        fetchPosts();
      } else { alert(data.message || 'Failed to create post'); }
    } catch { alert('Network error creating post'); }
    setPosting(false);
  };
  ...
  const handleSaveEdit = async () => {
    if (!editTitle.trim() && !editBody.trim()) { alert('Post cannot be empty.'); return; }
    ...
    try {
      const res = await fetch(`${API_BASE}/posts.php?action=edit_post`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ post_id: editingPost.id, title: editTitle, body: editBody }),
      });
      const data = await res.json();
      if (data.status === 'success') { setEditingPost(null); fetchPosts(); }
      else { alert(data.message || 'Failed to edit post'); }
  ...
  const handleDelete = async (postId: number) => {
    ...
    try {
      ...
      if (data.status === 'success') { setPosts(prev => prev.filter(p => p.id !== postId)); }
        ...
        if (data2.status === 'success') { setPosts(prev => prev.filter(p => p.id !== postId)); }
        else { alert(data2.message || 'Unable to delete post.'); }
      }
  ...
  const fetchComments = async (postId: number) => {
    const key = String(postId);
    try {
      const res = await fetch(`${API_BASE}/comments.php?action=get_comments&post_id=${postId}`, { credentials: 'include' });
      const data = await res.json();
  ...
  const handleSubmitComment = async (postId: number) => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        setPosts(prev => prev.map(p => p.id === postId ? { ...p, comments: (p.comments || 0) + 1, comment_count: ((p.comment_count || p.comments || 0) + 1) } : p));
      }
    } catch {}
  ...
  return (
    <div className="min-h-screen bg-[#fdfcfb]">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 gap-3 mb-6">
            <div className="relative">
              ...
              <input type="text" value={searchUser} onChange={e => setSearchUser(e.target.value)} placeholder="Search by user..." className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#d97706] bg-white" />
              {searchUser && <button onClick={() => setSearchUser('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#64748b] hover:text-[#1e293b]"><X className="size-4" /></button>}
            </div>
            <div className="relative">
              <Search className="size-4 text-[#64748b] absolute left-3 top-1/2 -translate-y-1/2" />
              <input type="text" value={searchTopic} onChange={e => setSearchTopic(e.target.value)} placeholder="Search by topic..." className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#d97706] bg-white" />
              {searchTopic && <button onClick={() => setSearchTopic('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#64748b] hover:text-[#1e293b]"><X className="size-4" /></button>}
            </div>
          </div>
              ...
              <div className="space-y-4">
                {loading && (
                  <div className="bg-white rounded-lg shadow-md p-8 text-center">
                    ...
                    <p className="text-sm text-[#64748b]">Loading forum posts...</p>
                  </div>
                )}
                {!loading && error && (
                ...
                {!loading && !error && filteredPosts.length === 0 && (
                  <div className="bg-white rounded-lg shadow-md p-8 text-center">
                    ...
                    {posts.length === 0 ? (
                      <>
                        <p className="font-medium text-[#1e293b] mb-1">No forum posts yet</p>
                        <p className="text-sm text-[#64748b] mb-4">Be the first to share something!</p>
                        <button onClick={() => setShowCreateModal(true)} className="px-6 py-2.5 bg-[#d97706] text-white rounded-lg font-medium text-sm">Create Post</button>
                      </>
                    ) : (
                      <>
                        <p className="font-medium text-[#1e293b] mb-1">No matching posts</p>
                      ...
                      <div className="flex items-start justify-between gap-3 mb-3">
                        ...
                        {isAuthor(post) && (
                          <div className="flex items-center gap-1 shrink-0">
                            <button onClick={() => startEdit(post)} className="p-1.5 text-[#64748b] hover:text-[#d97706] hover:bg-[#d97706]/10 rounded-lg transition-colors"><Pencil className="size-4" /></button>
                            <button onClick={() => handleDelete(post.id)} disabled={deletingId === post.id} className="p-1.5 text-[#64748b] hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-60"><Trash2 className="size-4" /></button>
                          </div>
                        )}
                      </div>
                            ...
                            {(postComments[postKey] || []).length === 0 ? (
                              ...
                              <div key={c.id} className="flex gap-2">
                                ...
                                <div className="flex-1 bg-gray-50 rounded-lg px-3 py-2">
                                  ...
                                  <p className="text-sm text-[#1e293b]">{c.text}</p>
                                </div>
                              </div>
                            ))}
                          </div>
            ...
            <div className="space-y-4">
              <div className="bg-white rounded-lg shadow-md p-4">
                ...
                <div className="space-y-3">
                  {groups.map(g => (
                    <div key={g.id} className="flex items-center justify-between p-3 bg-[#fdfcfb] rounded-lg hover:bg-gray-50 cursor-pointer transition-colors">
                      <div><p className="font-medium text-[#1e293b] text-sm">{g.name}</p><p className="text-xs text-[#64748b]">{g.members} members</p></div>
                      <Users className="size-4 text-[#d97706]" />
                    </div>
                  ))}
                </div>
                <button className="w-full mt-3 text-[#d97706] text-sm font-medium hover:text-[#b45309]">+ Join New Group</button>
              </div>

              <div className="bg-white rounded-lg shadow-md p-4">
                <h2 className="font-semibold text-[#1e293b] mb-3 flex items-center gap-2"><Calendar className="size-5 text-[#d97706]" />Upcoming Events</h2>
                <div className="space-y-3">
                  <div className="p-3 bg-[#fdfcfb] rounded-lg"><p className="font-medium text-[#1e293b] text-sm">Basketball Game</p><p className="text-xs text-[#64748b]">Today, 6:00 PM</p></div>
                  <div className="p-3 bg-[#fdfcfb] rounded-lg"><p className="font-medium text-[#1e293b] text-sm">Morning Run</p><p className="text-xs text-[#64748b]">Tomorrow, 6:30 AM</p></div>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Create Post Modal */}
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center" onClick={() => setShowCreateModal(false)}>
          <div className="bg-white rounded-t-2xl sm:rounded-lg w-full sm:max-w-lg p-4 sm:p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-center pt-1 pb-2 sm:hidden"><div className="w-10 h-1 bg-gray-300 rounded-full"></div></div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-[#1e293b]">Create Post</h2>
              <button onClick={() => setShowCreateModal(false)} className="text-[#64748b] hover:text-[#1e293b]"><X className="size-5" /></button>
            </div>
            <div className="space-y-3">
              <input type="text" value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="Post title (optional)" className="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#d97706] bg-white" />
              <textarea value={newBody} onChange={e => setNewBody(e.target.value)} placeholder="What's on your mind?" className="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#d97706] bg-white resize-none" rows={4} />
              {mediaPreview && (
                <div className="relative">
                  {mediaType === 'image' ? <img src={mediaPreview} alt="Preview" className="w-full max-h-48 object-cover rounded-lg" /> : <video controls className="w-full max-h-48 rounded-lg"><source src={mediaPreview} type="video/mp4" /></video>}
                  <button onClick={clearMedia} className="absolute top-2 right-2 bg-black/60 text-white rounded-full p-1"><X className="size-4" /></button>
                </div>
              )}
              <div className="flex items-center gap-2">
                <button onClick={() => { if (fileInputRef.current) { fileInputRef.current.accept = 'image/jpeg,image/jpg,image/png'; fileInputRef.current.click(); } }} className="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg text-sm text-[#64748b] hover:border-[#d97706] hover:text-[#d97706] transition-colors"><Image className="size-4" /> Image</button>
                <button onClick={() => { if (fileInputRef.current) { fileInputRef.current.accept = 'video/mp4'; fileInputRef.current.click(); } }} className="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg text-sm text-[#64748b] hover:border-[#d97706] hover:text-[#d97706] transition-colors"><Film className="size-4" /> Video</button>
                <input ref={fileInputRef} type="file" onChange={handleFileSelect} className="hidden" />
              </div>
            </div>
            <button onClick={handleCreatePost} disabled={posting || (!newTitle.trim() && !newBody.trim() && !newMedia)} className={`w-full mt-4 py-3 rounded-xl font-semibold text-sm text-white ${posting || (!newTitle.trim() && !newBody.trim() && !newMedia) ? 'bg-gray-300' : 'bg-[#d97706] hover:bg-[#b45309] active:scale-[0.98]'}`}>{posting ? 'Posting...' : 'Create Post'}</button>
          </div>
        </div>
      )}
      ...
      {/* Edit Post Modal */}
      {editingPost && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center" onClick={() => setEditingPost(null)}>
          <div className="bg-white rounded-t-2xl sm:rounded-lg w-full sm:max-w-lg p-4 sm:p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-center pt-1 pb-2 sm:hidden"><div className="w-10 h-1 bg-gray-300 rounded-full"></div></div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-[#1e293b]">Edit Post</h2>
              <button onClick={() => setEditingPost(null)} className="text-[#64748b] hover:text-[#1e293b]"><X className="size-5" /></button>
            </div>
            <div className="space-y-3">
              <input type="text" value={editTitle} onChange={e => setEditTitle(e.target.value)} placeholder="Post title" className="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#d97706] bg-white" />
              <textarea value={editBody} onChange={e => setEditBody(e.target.value)} placeholder="Post content" className="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#d97706] bg-white resize-none" rows={4} />
            </div>
            <div className="flex gap-3 mt-4">
              <button onClick={() => setEditingPost(null)} className="flex-1 py-3 border-2 border-gray-300 text-[#64748b] rounded-xl font-medium text-sm">Cancel</button>
              <button onClick={handleSaveEdit} disabled={saving || (!editTitle.trim() && !editBody.trim())} className={`flex-1 py-3 rounded-xl font-semibold text-sm text-white ${saving || (!editTitle.trim() && !editBody.trim()) ? 'bg-gray-300' : 'bg-[#d97706] hover:bg-[#b45309]'}`}>{saving ? 'Saving...' : 'Save'}</button>
            </div>
          </div>
        </div>
      )}
