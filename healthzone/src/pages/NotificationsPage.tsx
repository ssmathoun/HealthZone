...
export function NotificationsPage() {
  ...
  const fetchNotifications = () => {
    setLoading(true);
    fetch(`${API_BASE}/notifications.php?action=get_notifications`, {
      credentials: "include",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/notifications.php?action=mark_all_read`, {
      method: "POST",
      credentials: "include",
    }).catch(() => {});
  }, []);
  ...
  const handleClearAll = () => {
    fetch(`${API_BASE}/notifications.php?action=clear_notifications`, {
      method: "POST",
      credentials: "include",
    })
      .then((res) => res.json())
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm text-[#64748b]">
              You'll see alerts here when someone interacts with your posts
            </p>
          </div>
        ) : (
            ...
            {notifications.map((notif) => (
              ...
              >
                <div className="flex items-start gap-3">
                  <div
                    className={`size-10 rounded-full flex items-center justify-center shrink-0 ${notif.is_read ? "bg-gray-100" : "bg-[#d97706]/10"}`}
                  >
                    <MessageCircle
                      className={`size-5 ${notif.is_read ? "text-[#64748b]" : "text-[#d97706]"}`}
                    />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-sm ${notif.is_read ? "text-[#64748b]" : "text-[#1e293b] font-medium"}`}
                    >
