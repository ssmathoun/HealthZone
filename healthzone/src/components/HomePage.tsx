  ...
  return (
    <div className="min-h-screen bg-[#fdfcfb]">
          ...
          <div className="flex items-center gap-1">
            ...
            <button className="text-white p-2 hover:bg-white/10 rounded-full" onClick={() => {
              fetch('https://aptitude.cse.buffalo.edu/CSE442/2026-Spring/cse-442v/php/logout.php', { method: 'POST', credentials: 'include' })
                .finally(() => { window.location.hash = '/login'; });
            }}>
      ...
      <main className="px-4 py-4 max-w-4xl mx-auto">
        <div className="mb-4">
          <h1 className="text-xl sm:text-2xl font-bold text-[#1e293b] mb-1">Welcome back! 💪</h1>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <StatCard icon={<Activity className="size-5 text-[#d97706]"/>} label="Calories" value={stats.caloriesConsumed} goal={stats.caloriesGoal} percentage={stats.caloriesGoal > 0 ? Math.round((stats.caloriesConsumed / stats.caloriesGoal) * 100) : 0} />
          <StatCard icon={<Heart className="size-5 text-[#d97706]"/>} label="Protein" value={stats.proteinsConsumed} goal={stats.proteinsGoal} percentage={stats.proteinsGoal > 0 ? Math.round((stats.proteinsConsumed / stats.proteinsGoal) * 100) : 0} />
          <button 
          ...
          >
            <div className="flex items-center gap-1.5 mb-1.5 sm:mb-2">
              ...
              <span className="text-[10px] sm:text-xs text-[#64748b]">Workouts</span>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-4 mb-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigate('/log-meal')} className="text-xs text-[#d97706] font-medium">+ Add</button>
          </div>
          {loggedMeals.length === 0 ? (
            ...
            <div className="space-y-2">
              {loggedMeals.map((meal) => (
                <div key={meal.id} className="flex items-center justify-between p-2.5 sm:p-3 border border-gray-200 rounded-lg gap-2">
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-[#1e293b] text-sm truncate">{meal.name}</p>
                    <p className="text-[10px] sm:text-xs text-[#64748b] capitalize">{meal.meal_type || meal.type} • {meal.protein}g P • {meal.carbs}g C • {meal.fat}g F</p>
                  </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-4 mb-4">
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={activity.name} className="flex items-start gap-2.5 sm:gap-3 p-2.5 sm:p-3 border border-gray-200 rounded-lg hover:border-[#d97706] transition-colors cursor-pointer">
                <div className="text-xl sm:text-2xl flex-shrink-0">{activity.icon}</div>
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-[#1e293b] text-sm truncate">{activity.name}</p>
                  <p className="text-[10px] sm:text-xs text-[#64748b] truncate">{activity.group} • {activity.time}</p>
                  <p className="text-[10px] sm:text-xs text-[#64748b] flex items-center gap-1 mt-0.5 truncate"><MapPin className="size-3 flex-shrink-0" />{activity.location} • {activity.participants} going</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {showWorkoutModal && !selectedWorkout && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center" onClick={() => setShowWorkoutModal(false)}>
      ...
      {selectedWorkout && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center" onClick={resetWorkout}>
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-2.5 sm:p-3">
      ...
      <div className="text-sm sm:text-lg font-bold text-[#1e293b] mb-1">
        {value}{unit} <span className="text-[10px] sm:text-xs font-normal text-[#64748b]">/ {goal}{unit}</span>
      </div>
      <div className="w-full bg-gray-200 rounded-full h-1.5">
        <div className="bg-[#d97706] h-1.5 rounded-full transition-all duration-300" style={{ width: `${Math.min(percentage, 100)}%` }}></div>
      </div>
      <div className="text-[10px] sm:text-xs text-[#64748b] mt-1">{percentage}%</div>
    </div>
  );
}