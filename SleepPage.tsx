...
import { ArrowLeft, Moon, Save, History, Clock, BarChart2, Share2 } from 'lucide-react';
        ...
        {history.length > 0 && (
          <button
            onClick={() => {
              ...
              const avg = history.length > 0 ? (history.reduce((sum, h) => sum + Number(h.hours), 0) / history.length).toFixed(1) : '0';
              const text = `HealthZone Sleep Update!\nLast night: ${Number(latest.hours).toFixed(1)} hrs\nAverage: ${avg} hrs over ${history.length} nights\n#HealthZone #SleepTracking`;
              if (navigator.share) {
                navigator.share({ title: 'My HealthZone Sleep Progress', text }).catch(() => {});
              } else {
                navigator.clipboard.writeText(text).then(() => alert('Sleep progress copied to clipboard! Paste it on your social media.')).catch(() => {});
              }
            }}
            className="w-full bg-[#1e293b] text-white py-3 rounded-xl mb-6 font-bold flex items-center justify-center gap-2 hover:bg-[#334155] transition-colors"
          >
